...
export default async function handler(req,res){
  ...
  try{
    ...
    const chats=asArray(chatsData.chats).filter(c=>{const changed=Number(c.updated||c.created||0);return !changed||changed*1000>=safeSince.getTime()}).map(c=>{const customer=c.users?.find?.(u=>String(u.id)!==account()),lastInbound=String(c.last_message?.author_id||'')!==account(),unreadCount=Number(c.unread_count||0);return{id:`chat:${c.id}`,kind:'message',externalId:String(c.id),itemId:String(c.context?.value?.id||c.item_id||''),createdAt:c.updated||c.created||'',name:customer?.name||'',title:customer?.name||c.context?.value?.title||'Клиент Avito',preview:c.last_message?.content?.text||(c.last_message?.content?.voice?.voice_id?'Голосовое сообщение':''),unreadCount,lastInbound,attention:lastInbound&&unreadCount>0}});
