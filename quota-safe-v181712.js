...
(function(){
  ...
  async function installPatch(cloud){
    ...
    let googleCfg={},googleUnsub=null,pullTimer=null,pushTimer=null,bridgeFlushing=false,bridgeRetryAttempt=0,baselineMap={};
    ...
    function enqueueBridgeRows(rows){
      for(const r0 of rows||[]){
        ...
        if(id&&!TECH_RE.test(id)&&!knownDeleted.has(id)&&!pushDeletes.has(id))pushRows.set(id,r);
      }
      scheduleBridgeFlush();
    }
    ...
    async function flushBridge(){
      ...
      try{
        ...
        let failed=0,okRows=0,okDeletes=0,lastErr='';
        for(const [id,item] of dels){
          try{
            ...
            delete baselineMap[id];okDeletes++;diag.bridgeDeletes++;
