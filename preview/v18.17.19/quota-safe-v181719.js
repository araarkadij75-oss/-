/* MASTER AI 18.17.19 — Spark quota-safe realtime / Bridge Protocol v2 + canonical delete safety.
   Loaded synchronously from cloud-config.js before the embedded cloud adapter.
   No production endpoints, no billing, no secrets. */
(function(){
  'use strict';
  const PATCH='18.17.19-certification';
  ...
  async function installPatch(cloud){
    ...
    async function manualReconcileV2(opts={}){
      ...
      try{
        ...
        const activate=String(window.MASTER_AI_BUILD||'').includes('18.17.19-CANDIDATE'),endpoint=bridgeEndpoint(g);
    ...
    cloud.subscribe=function(profile,onRows){
      ...
      return (async()=>{
        ...
        googleUnsub=fs.onSnapshot(googleRef,d=>{
          ...
          if(role==='owner'&&String(window.MASTER_AI_BUILD||'').includes('18.17.19-CANDIDATE')&&googleCfg.bridgeUrlV2&&googleCfg.bridgeUrl){
