/* MASTER AI 18.17.18 — Spark quota-safe realtime / Bridge Protocol v2 + canonical delete safety.
   Loaded synchronously from cloud-config.js before the embedded cloud adapter.
   No production endpoints, no billing, no secrets. */
(function(){
  'use strict';
  const PATCH='18.17.18-delete-safety';
  ...
  async function installPatch(cloud){
    ...
    async function refreshGoogleCfg(){
      try{
        ...
        if(cloud.profile?.role==='owner'){
          const legacy=String(x.bridgeUrl||''),v2=String(x.bridgeUrlV2||''),activate=String(window.MASTER_AI_BUILD||'').includes('18.17.18-CURRENT');
          if(!v2&&legacy){
    ...
    async function reapTechnicalTombstones(){
      ...
      if(resolved.length){
        ...
        for(const id of resolved)delete baselineMap[id];
        await fs.setDoc(googleRef,{syncBaseline:baselineMap,syncBaselineVersion:1},{merge:true});
        googleCfg={...g,deletedOrders:(Array.isArray(g.deletedOrders)?g.deletedOrders:[]).filter(x=>!dead.has(String(x&&x.orderId||''))),syncBaseline:baselineMap,syncBaselineVersion:1};
    ...
    async function persistOwnerDeleteTombstones(items){
      const incoming=(items||[]).map(x=>({orderId:String(x&&x.orderId||'').trim(),deletedAt:String(x&&x.deletedAt||new Date().toISOString()),source:'crm'})).filter(x=>x.orderId);
      if(!incoming.length)return[];
      const g=(googleCfg&&Object.keys(googleCfg).length)?googleCfg:await refreshGoogleCfg();
      const map=new Map((Array.isArray(g.deletedOrders)?g.deletedOrders:[]).map(x=>[String(x&&x.orderId||''),x]).filter(x=>x[0]));
      for(const x of incoming)map.set(x.orderId,map.get(x.orderId)||x);
      const deletedOrders=[...map.values()].slice(-500);
      await fs.setDoc(googleRef,{deletedOrders,lastTombstoneAt:new Date().toISOString()},{merge:true});
      googleCfg={...g,deletedOrders};
      return incoming;
    }
    async function deleteCanonicalAndMirrors(item){
      ...
      const aux=await Promise.allSettled([
        ...
        fs.deleteDoc(fs.doc(db,'workspaces',ws,'masterPhones',id))
      ]);
      const names=['dispatcherOrders','masterAssignments','masterPhones'],auxFailures=[];
      aux.forEach((r,i)=>{if(r.status==='rejected')auxFailures.push({collection:names[i],error:String(r.reason&&r.reason.message||r.reason||'delete failed').slice(0,240)})});
      diag.auxDeleteFailures+=auxFailures.length;
      delete baselineMap[id];
      knownDeleted.add(id);pushRows.delete(id);
      return{ok:true,id,auxFailures};
    }
    let ownerDeleteChain=Promise.resolve();
    async function processOwnerDeletes(items){
      const normalized=(items||[]).map(x=>({orderId:String(x&&x.orderId||'').trim(),deletedAt:String(x&&x.deletedAt||new Date().toISOString()),source:'crm'})).filter(x=>x.orderId);
      const fresh=normalized.filter(x=>!knownDeleted.has(x.orderId));
      if(!fresh.length)return{ok:true,processed:0,auxFailures:[]};
      try{
        await persistOwnerDeleteTombstones(fresh);
        await rememberDeletes(fresh);
        const results=[];
        for(const x of fresh)results.push(await deleteCanonicalAndMirrors(x));
        enqueueBridgeDeletes(fresh);
        diag.deleteErrors=0;diag.deleteDeadLetter=false;diag.lastDeleteError='';
        return{ok:true,processed:results.length,auxFailures:results.flatMap(x=>x.auxFailures||[])};
      }catch(e){
        diag.deleteErrors++;diag.lastDeleteError=String(e&&e.message||e).slice(0,500);diag.deleteDeadLetter=diag.deleteErrors>=5;
        try{await persistDeltaState({deleteFailureAt:new Date().toISOString(),deleteError:diag.lastDeleteError,deleteDeadLetter:diag.deleteDeadLetter})}catch(_){}
        throw e;
      }
    }
    function queueOwnerDeletes(items){
      ownerDeleteChain=ownerDeleteChain.then(()=>processOwnerDeletes(items)).catch(e=>{
        console.error('owner canonical delete deferred',e);
        window.MasterAICloudStatus&&window.MasterAICloudStatus('error','Удаление не завершено: '+String(e&&e.message||e));
      });
      return ownerDeleteChain;
    }
    ...
    async function manualReconcileV2(opts={}){
      ...
      try{
        ...
        const activate=String(window.MASTER_AI_BUILD||'').includes('18.17.18-CURRENT'),endpoint=bridgeEndpoint(g);
    ...
    cloud.subscribe=function(profile,onRows){
      ...
      return (async()=>{
        ...
        googleUnsub=fs.onSnapshot(googleRef,d=>{
          ...
          if(role==='owner'&&String(window.MASTER_AI_BUILD||'').includes('18.17.18-CURRENT')&&googleCfg.bridgeUrlV2&&googleCfg.bridgeUrl){
        ...
        /* Full Google reconciliation is manual-only in 18.17.18.
    ...
    if(originalQueue){
      cloud.queueSync=function(orders,deleted,settings,role){
        const roleName=String(role||''),owner=roleName==='owner',allDeleted=Array.isArray(deleted)?deleted:[];
        /* Owner deletions are handled exclusively by the quota-safe path.
           Passing [] prevents legacy Promise.all deletes / duplicate tombstone processing. */
        const ret=originalQueue(orders,owner?[]:deleted,settings,role);
        if(owner&&allDeleted.length)queueOwnerDeletes(allDeleted);
        return ret;
      };
    }
    ...
    cloud.saveGoogle=async function(x={}){
      ...
      const cur=await refreshGoogleCfg(),endpoint=String(x.bridgeUrlV2||x.bridgeUrl||bridgeEndpoint(cur)||'').trim(),activate=String(window.MASTER_AI_BUILD||'').includes('18.17.18-CURRENT');
    ...
    if(originalHealth){
      cloud.integrationHealth=async function(){
        ...
        h.syncState={...(h.syncState||{}),deletionsPending:(Array.isArray(googleCfg.deletedOrders)?googleCfg.deletedOrders.length:0),baselineCount:Object.keys(baselineMap).filter(id=>!TECH_RE.test(id)).length,technicalBaselineIgnored:Object.keys(baselineMap).filter(id=>TECH_RE.test(id)).length,quotaSafeReads:true,deltaRealtime:['delta','bootstrap_delta'].includes(diag.mode),deltaCursor:diag.lastCursor,cacheRows:diag.cacheRows,deltaReads:diag.deltaReads,bridgeMode:'direct_upsert_delete',bridgeDeltaPushes:diag.bridgePushes,bridgeDeltaDeletes:diag.bridgeDeletes,bridgeDeltaErrors:diag.bridgeErrors,bridgePendingRows:pushRows.size,bridgePendingDeletes:pushDeletes.size,deltaRetryCount:diag.deltaRetryCount,deltaDeadLetter:diag.deltaDeadLetter,recoveredDirty:diag.recoveredDirty,lockDeferrals:diag.lockDeferrals,fullReconcileMode:'manual_only',legacyAutoFullSyncBlocked:true,autoCoordinator:['owner'],googleCoordinator:['owner'],autoFlushOnOpen:false,autoFlushDirect:true,eventDriven:true,queueCoalescing:true,focusPull:false,retryLimit:5,busyRetryLimit:0,pendingDurableDeletes:(Array.isArray(googleCfg.deletedOrders)?googleCfg.deletedOrders.length:0),integrationE2EBridge:true,technicalTombstoneCleanup:true,canonicalDeleteSafety:true,canonicalDeletes:diag.canonicalDeletes,auxDeleteFailures:diag.auxDeleteFailures,deleteErrors:diag.deleteErrors,deleteDeadLetter:diag.deleteDeadLetter,lastDeleteError:diag.lastDeleteError,bridgeProtocol:2,legacyBridgeEndpointDisabled:!!googleCfg.bridgeUrlV2&&!googleCfg.bridgeUrl,manualReconcileCanonical:'firestore_to_google',patch:PATCH};
        return h;
      };
    }
    ...
    window.MASTER_AI_BUILD='18.17.18-CANDIDATE';
